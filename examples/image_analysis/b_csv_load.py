# Step B: CSV load + naive numpy colorcode.

# Loading four large CSVs via ``np.loadtxt`` dominates wall time no matter how
# naive or optimized the compute underneath is: every profiler's top hotspot is
# the call-dense CSV parse (and a tracing profiler's overhead shows on it too).
